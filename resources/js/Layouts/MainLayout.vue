      ...
      <nav class="p-4 flex items-center justify-between">
        ...
        <div v-if="user" class="flex items-center gap-4">
          <Link :href="route('notifications.index')" class="text-gray-500 relative pr-2 py-2 text-lg">
            🔔
            <div v-if="notificationsCount" class="absolute right-0 top-0 w-5 h-5 bg-red-700 dark:bg-red-400 text-white font-medium border border-white dark:border-gray-900 rounded-full text-xs text-center">
              {{ notificationsCount }}
            </div>
          </Link>
